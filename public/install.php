...
<div class="footer">
  ...
  <h6>版权所有 2020-$_date &copy; <a href="https://www.moguyun.pro" target="_blank">$site_name</a></h6>
</div>
</body>
</html>
EOF;

if (empty($_GET['step'])||! in_array($_GET['step'], array(1, 2, 3, 4, 5))) {
    $_GET['step'] = 0;
}
...
EOF;
    ...
    case 3:
        ...
        if($_SERVER['REQUEST_METHOD']=='POST'){
            ...
            $db_name = $_POST['db_name']??'moguyun';
            $db_user = $_POST['db_user']??'root';
            $db_pwd = $_POST['db_pwd']??'123456';
...
<div class="main">
  ...
  <div class="final-site-nav">
    ...
    <ul>
      ...
      <li class="admin">
        <div class="ico"></div>
        <h5><a href="{$auto_site_url}/admin" target="_blank">后台</a></h5>
        <h6>系统后台</h6>
      </li>
    </ul>
  </div>
  <div class="final-intro">
    <p><strong>系统管理默认地址:&nbsp;</strong><a href="{$auto_site_url}/admin" target="_blank">{$auto_site_url}/admin</a></p>
...
EOF;
    ...
    // curl(base64_decode('aHR0cDovL3VwZ3JhZGUuanN1cGhwLmNvbQ=='),['method'=>'upgrade://api/install','domain'=>$_SERVER['HTTP_HOST']]);
    deldir('../runtime/');
        echo $step;
        break;
    default:
...
<div class="main">
  <div class="text-box" id="text-box">
    <div class="license">
      <h1>系统安装协议</h1>
      <p>感谢您选择本系统。本系统是自主开发、集资讯、商城、讨论组、微商城、在线IM、客户管理于一体的门户型解决方案。官方网址为 https://www.moguyun.pro。</p>
...
EOF;
    ...
    $auto_site_url = strtolower($_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'].$sitepath);
    ...
    $pdo->query("UPDATE admin SET username = '{$admin}', password = '{$newPassword}' ,salt='{$salt}' WHERE username = 'admin'");


    //新增一个标识文件，用来屏蔽重新安装
    $fp = @fopen('../config/install.lock', 'wb+');
    @fclose($fp);
    exit("<script type=\"text/javascript\">document.getElementById('install_process').innerHTML = '安装完成，下一步...';document.getElementById('install_process').href='install.php?step=5';</script>");
    exit();
}
...
function env_check(&$env_items)
{
    ...
    $env_items[] = array(
        'name'   => 'PHP版本', 'min' => '7.1', 'good' => '7.3', 'cur' => PHP_VERSION,
        'status' => (PHP_VERSION < 7.1 ? 0 : 1)
    );
