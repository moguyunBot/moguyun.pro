...
abstract class Addons
{
    ...
    {
        $this->assign('base',root_path().'view/base.html');
    }
    ...
    protected function create_menu($menus,$pid=0){
        foreach($menus as $v){
            $v['pid'] = $pid;
            $v['addon_name'] = $this->name;
            $menu = \app\common\model\Rule::create($v);
            if(!empty($v['sublist'])){
                $this->create_menu($v['sublist'],$menu['id']);
            }
        }
    }
    
    protected function delete_menu(){
        \app\common\model\Rule::where(['addon_name'=>$this->name])->delete();
    }
}
